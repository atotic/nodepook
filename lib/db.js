	...
	function saveToDb() {
		promise.when( 
			promise.seq([
		 	...
		 	function error(err) {
		 		AWSu.s3.deleteObject( 'photos', attributes.s3id);
...
function deletePhoto(sdbId) {
	return promise.seq([
		...
		function deleteS3(data) {
			return promise.all([
				AWSu.s3.deleteObject( 'photos', data.s3id),
				AWSu.s3.deleteObject( 'photos', data.s3id + photoUtil.separator + "256"),
				AWSu.s3.deleteObject( 'photos', data.s3id + photoUtil.separator + "1024")
				]);
		},
		function deleteSdb() {
